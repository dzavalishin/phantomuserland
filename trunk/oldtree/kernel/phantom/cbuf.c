...
static hal_mutex_t cbuf_free_list_lock;
static cbuf *cbuf_free_noblock_list;
static hal_spinlock_t noblock_spin;

static hal_spinlock_t cbuf_lowlevel_spinlock;
//static region_id cbuf_region_id;
static cbuf *cbuf_region;
//static region_id cbuf_bitmap_region_id;
static u_int8_t *cbuf_bitmap;
...
void cbuf_free_chain(cbuf *buf)
{
    ...
    hal_mutex_lock(&cbuf_free_list_lock);

    last->next = cbuf_free_list;
    cbuf_free_list = head;

    hal_mutex_unlock(&cbuf_free_list_lock);
}
    ...
    while(chain_len < len) {
        if(cbuf_free_list == NULL) {
            ...
            if(!temp) {
                ...
                printf("cbuf_get_chain: asked to allocate %ld bytes but out of memory\n", len);
                return NULL;
            }
...
static u_int16_t _cbuf_ones_cksum16(cbuf *buf, size_t offset, size_t len, u_int16_t sum)
{
    int swapped = 0;
...
{
    u_int16_t sum = ones_sum16(0, buf, buf_len);
...
int cbuf_init()
{
    ...
    //cbuf_bitmap_region_id = vm_create_anonymous_region(vm_get_kernel_aspace_id(), "cbuf bitmap region",(void **)&cbuf_bitmap, REGION_ADDR_ANY_ADDRESS,CBUF_BITMAP_SIZE / 8, REGION_WIRING_WIRED, LOCK_RW|LOCK_KERNEL);

...
void draw_block (int h, int w, int visible)
{
	...
	if (visible) {
		...
		if (h == (PITDEPTH-1)*5)
			drv_video_window_pixel ( tetris_window, h + 4, w + 2, COLOR_LIGHTRED);

		if (w == 0)
			drv_video_window_pixel ( tetris_window, h + 2, w, COLOR_LIGHTRED);
		else if (w % 20 == 15)
			drv_video_window_pixel ( tetris_window, h + 2, w + 4, COLOR_LIGHTRED);
	}
}
...
void init_tetris ()
{
	...
	hal_start_kernel_thread(tetris_window_loop);

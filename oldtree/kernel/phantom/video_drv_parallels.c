...
void SVGA_Panic(const char *s) { panic("parallels SVGA: %s", s); }
...
static void parallels_map_video(int on_off)
{
    ...
    hal_pages_control( gSVGA.fbMem, video_driver_parallels_svga.screen, N_PAGES, on_off ? page_map_io : page_unmap, page_rw );

}

static int parallels_video_start()
{
    parallels_detect2( video_driver_parallels_svga.xsize, video_driver_parallels_svga.ysize, video_driver_parallels_svga.bpp );

    switch_screen_bitblt_to_32bpp(1);
    parallels_map_video( 1 );

    // In fact, detect already set it - here we init fifo
    SVGA_SetMode( video_driver_parallels_svga.xsize, video_driver_parallels_svga.ysize, video_driver_parallels_svga.bpp );
...
static errno_t parallels_accel_start(void)
{
    ...
    parallels_detect2( video_drv->xsize, video_drv->ysize, video_drv->bpp );

    // In fact, detect already set it - here we init fifo
    SVGA_SetMode( video_drv->xsize, video_drv->ysize, video_drv->bpp );

    video_drv->update = parallels_video_update;

#if 1 // doesn't work
    SHOW_FLOW( 0, "parallels accelerator add on start, id %x", gSVGA.deviceVersionId );
    ...
    if( gSVGA.capabilities & SVGA_CAP_CURSOR )
    {
        ...
        {
            // Take over mouse
            video_drv->mouse_redraw_cursor = parallels_draw_mouse_bp2;
            video_drv->mouse_set_cursor    = parallels_set_mouse_cursor_bp2;
            video_drv->mouse_disable       = parallels_mouse_off_bp2;
            video_drv->mouse_enable        = parallels_mouse_on_bp2;
        }
    ...
    parallels_map_video(1);
    video_drv->screen = video_driver_parallels_svga.screen;
    */
    return 0;
}
...
phantom_device_t * driver_parallels_svga_pci_probe( pci_cfg_t *pci, int stage )
{
    ...
    if (gSVGA.deviceVersionId >= SVGA_ID_1) {
        ...
        if( gSVGA.capabilities & SVGA_CAP_CURSOR )
        {
            ...
            if(cpb2)
            {
                video_driver_parallels_svga.mouse_redraw_cursor = parallels_draw_mouse_bp2;
                video_driver_parallels_svga.mouse_set_cursor    = parallels_set_mouse_cursor_bp2;
                video_driver_parallels_svga.mouse_disable       = parallels_mouse_off_bp2;
                video_driver_parallels_svga.mouse_enable        = parallels_mouse_on_bp2;
            }
...
SVGA_FIFOReserve(u_int32_t bytes)  // IN
{
   ...
   if (bytes > sizeof gSVGA.fifo.bounceBuffer ||
       bytes > (max - min)) {
       panic("parallels: FIFO command too large (%d > %d)", bytes, sizeof gSVGA.fifo.bounceBuffer );
   }
 ...
 *      like video overlays, for parallels's internal debugging tools,
...
static void parallels_video_update(void)
{
    ...
    SVGA_Update( 0, 0, video_driver_parallels_svga.xsize, video_driver_parallels_svga.ysize );
}
...
void parallels_draw_mouse_bp2(void)
{
    SVGA_WriteReg(SVGA_REG_CURSOR_X, video_driver_parallels_svga.mouse_x );
    SVGA_WriteReg(SVGA_REG_CURSOR_Y, scr_get_ysize() - video_driver_parallels_svga.mouse_y );

...
static void fat_dump_dirent( struct fat_dir *f )
{
	...
	printf("LoCluster: %d\n", f->lo_cluster );
	printf("HiCluster: %d\n", f->hi_cluster );
	printf("Attrs    : %b\n", f->attribute, "\020\1RO\2HIDDEN\3SYSTEM\4VOL\5DIR\6ARCH\7DEV\8UNUSED" );
}
...
{
	if(cluster_no < 2 || cluster_no > fs->max_cluster_no)
		return -1;
...
{
	int chain = dir->lo_cluster | (dir->hi_cluster << 16);

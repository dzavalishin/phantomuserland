...
{
    // -----------------------------------
    // Phantom driver's workspace
    // -----------------------------------

    int                 	driver_busmaster;
    ...
    struct disk_q *		q;              // q we got rq from
    //disk_page_no_t       	disk;           // Disk SECTOR number (incoming parameter)
    unsigned long		startSector;    // Disk SECTOR (512b) number (incoming parameter)
    unsigned int		nSectors;    	// Number of SECTORS to transfer (incoming parameter)
    phys_page_t         	mem;            // Mem phys address
    int                         devSelect;      // Device for current op
    ...
    phantom_disk_partition_t *  dp0;
    phantom_disk_partition_t *  dp1;

    // called when IO code waits for interrupt.
    // Driver wrapper gives out CPU here

    void			(*intr_wait)( struct ataio *ata );

    // Called from interrupt to awake above func

    void			(*intr_awake)( struct ataio *ata );

    // -----------------------------------
    // Rest is internal ATA vars, quite a junk
    // -----------------------------------
    ...
    struct REG_CMD_INFO 	global_reg_cmd_info;
#define reg_cmd_info (ata->global_reg_cmd_info)
    ...
    // must be some part of internal communications of busmaster functions
...
extern int dma_pci_config( ataio_t *ata, unsigned int regAddr );

extern void dma_pci_set_max_xfer( ataio_t *ata, unsigned int seg, unsigned int off,
                                  long bufSize );
/*
extern int dma_pci_chs( int dev, int cmd,
                        ...
                        long numSect );
*/
extern int dma_pci_lba28( ataio_t *ata, int dev, int cmd,
                          unsigned int fr, unsigned int sc,
                          unsigned long lba,
                          //unsigned int seg, unsigned int off,
                          physaddr_t physaddr,
...
extern int dma_pci_lba48( ataio_t *ata, int dev, int cmd,
                          unsigned int fr, unsigned int sc,
                          unsigned long lbahi, unsigned long lbalo,
                          physaddr_t physAddr,
                          //unsigned int seg, unsigned int off,
...
extern int dma_pci_packet( ataio_t *ata, int dev,
                           unsigned int cpbc,
                           //unsigned int cpseg, unsigned int cpoff,
...
extern void pio_drq_block_in( ataio_t *ata, unsigned int addrDataReg,
                              void *addr,
                              long wordCnt );

extern void pio_drq_block_out( ataio_t *ata, unsigned int addrDataReg,
                              void *addr,
                               long wordCnt );
...
extern int reg_config( ataio_t *ata  );

extern int reg_reset( ataio_t *ata, int skipFlag, int devRtrn );

// ATA Non-Data command funnctions (for CHS, LBA28 and LBA48)

extern int reg_non_data_chs( ataio_t *ata, int dev, int cmd,
                             unsigned int fr, unsigned int sc,
                             unsigned int cyl, unsigned int head, unsigned int sect );

extern int reg_non_data_lba28( ataio_t *ata, int dev, int cmd,
                               unsigned int fr, unsigned int sc,
                               unsigned long lba );

extern int reg_non_data_lba48( ataio_t *ata, int dev, int cmd,
                               unsigned int fr, unsigned int sc,
                               unsigned long lbahi, unsigned long lbalo );
...
extern int reg_pio_data_in_chs( ataio_t *ata, int dev, int cmd,
                                unsigned int fr, unsigned int sc,
                                unsigned int cyl, unsigned int head, unsigned int sect,
                                unsigned int seg, unsigned int off,
                                long numSect, int multiCnt );

extern int reg_pio_data_in_lba28( ataio_t *ata, int dev, int cmd,
                                  unsigned int fr, unsigned int sc,
                                  unsigned long lba,
                                  //unsigned int seg, unsigned int off,
                                  void *addr,
...
extern int reg_pio_data_in_lba48( ataio_t *ata, int dev, int cmd,
                                  unsigned int fr, unsigned int sc,
                                  unsigned long lbahi, unsigned long lbalo,
                                  //unsigned int seg, unsigned int off,
                                  void *addr,
...
extern int reg_pio_data_out_chs( ataio_t *ata, int dev, int cmd,
                                 unsigned int fr, unsigned int sc,
                                 unsigned int cyl, unsigned int head, unsigned int sect,
                                 unsigned int seg, unsigned int off,
                                 long numSect, int multiCnt );

extern int reg_pio_data_out_lba28( ataio_t *ata, int dev, int cmd,
                                   unsigned int fr, unsigned int sc,
                                   unsigned long lba,
                                   //unsigned int seg, unsigned int off,
                                   void *addr,
...
extern int reg_pio_data_out_lba48( ataio_t *ata, int dev, int cmd,
                                   unsigned int fr, unsigned int sc,
                                   unsigned long lbahi, unsigned long lbalo,
                                   void *addr,
                                   long numSect, int multiCnt );
...
extern int reg_packet( ataio_t *ata, int dev,
                       unsigned int cpbc,
                       void *cpAddr,
                       //unsigned int cpseg, unsigned int cpoff,
                       int dir,
...
extern void tmr_delay_1ms( int count );

//extern void tmr_delay_1us( long count );

extern void tmr_delay_ata( void );

extern void tmr_delay_atapi( ataio_t *ata, int dev );
...
extern const char * trc_err_dump2( ataio_t *ata );
...
#define ATAPI_DELAY(ata,dev) {pio_inbyte(CB_ASTAT);tmr_delay_atapi(ata,dev);pio_inbyte(CB_ASTAT);}
...
extern void sub_zero_return_data( ataio_t *ata );
extern void sub_setup_command( ataio_t *ata );
extern void sub_trace_command( ataio_t *ata );
extern int sub_select( ataio_t *ata, int dev );
extern unsigned char sub_readBusMstrCmd( ataio_t *ata );
extern unsigned char sub_readBusMstrStatus( ataio_t *ata );
extern void sub_writeBusMstrCmd( ataio_t *ata, unsigned char x );
extern void sub_writeBusMstrStatus( ataio_t *ata, unsigned char x );
...
extern void trc_cht( ataio_t *ata );
...
#define pio_inbyte( addr ) \
({ \
    unsigned int a = addr; 			\
    unsigned char uc;                           \
                                                \
    uc = (unsigned char) inb( (int)ata->pio_reg_addrs[ a ] );        \
    ata->pio_last_read[ a ] = uc;            	\
    trc_llt( a, uc, TRC_LLT_INB );      	\
    uc;                                         \
    })



#define pio_inword( addr ) \
({                                              \
    unsigned int a = addr;                      \
    unsigned int ui;                            \
                                                \
    ui = inw( (int)ata->pio_reg_addrs[ a ] );   \
    trc_llt( a, 0, TRC_LLT_INW );            	\
    ui;                                         \
})



#define pio_outbyte( addr, data ) \
({                                              \
    unsigned char uc = (data);                  \
    unsigned int a = (addr);                    \
    outb( (int)ata->pio_reg_addrs[ a ], uc );   \
    ata->pio_last_write[ a ] = uc;              \
    trc_llt( a, uc, TRC_LLT_OUTB );             \
})



#define pio_outword( addr, data ) \
({                                              \
    unsigned int a = (addr);                    \
    outw( (int)ata->pio_reg_addrs[ a ], data ); \
    trc_llt( a, 0, TRC_LLT_OUTW );              \
})
...
#define  pio_rep_inbyte( addrDataReg, addr, byteCnt ) \
({                                                              \
    unsigned int a = (addrDataReg);                    		\
    insb((int)ata->pio_reg_addrs[ a ], addr, byteCnt );       	\
    trc_llt( a, 0, TRC_LLT_INSB );                            	\
})



#define  pio_rep_outbyte( addrDataReg, addr, byteCnt )          \
({                                                              \
    unsigned int a = (addrDataReg);                    		\
    outsb((int)ata->pio_reg_addrs[ a ], addr, byteCnt);         \
    trc_llt( a, 0, TRC_LLT_OUTSB );                             \
})



#define  pio_rep_inword( addrDataReg, addr, wordCnt )           \
({                                                              \
    unsigned int a = (addrDataReg);                    		\
    insw((int)ata->pio_reg_addrs[ a ], addr, (int)wordCnt );    \
    trc_llt( a, 0, TRC_LLT_INSW );                              \
})



#define  pio_rep_outword( addrDataReg, addr, wordCnt ) 		\
{                                                               \
    unsigned int a = (addrDataReg);                    		\
    outsw((int)ata->pio_reg_addrs[ a ], addr, (int)wordCnt );   \
    trc_llt( a, 0, TRC_LLT_OUTSW );                             \
}

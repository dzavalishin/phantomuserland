...
static int parse_cast(scan_info * info,shell_value **out)
{
    ...
    long int dummy;
    char *text;
    int to_number = info->sym_code == SVO_NUMBER_CAST;
...
void ste_error_to_str(int error,char *descr, int size)
{
    ...
    if(error <= SVO_MAX){
        snprintf(descr, size, "'%s' expected",id_to_token(error));
        return;
    }
    switch(error){
    case SHE_NO_ERROR:
...
int get_path( int no, char *name, int max_len)
{
    ...
    char *value;
    int err;
    if(var_value == NULL) return 0;

    err = shell_value_get_text(var_value,&value);
    if(err != SHE_NO_ERROR) return 0;
...
static int handle_ident(scan_info *info,int *is_label)
{
...
static int is_label_with_name(scan_info *info,char *check_label ,int *is_label)
{
    ...
    *is_label = 0;
    ...
    if(*scanner == ':'){
        ...
        if(strcmp(check_label , info->token) == 0) *is_label = 1;
...
int parse_info(scan_info *info)
{
    ...
    if(err != SHE_NO_ERROR){
        ...
        ste_error_to_str(err,err_txt, sizeof(err_txt));
...
void init_statements(void){
    ...
    shell_var_set_text(NAME_VAR_PATH,".:/:/boot:/boot/bin:/amnt0:/amnt0/bin:/amnt1:/amnt1/bin:/amnt2:/amnt2/bin:/amnt0/etc:/amnt1/etc:/amnt2/etc");

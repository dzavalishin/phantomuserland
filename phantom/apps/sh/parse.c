...
static int  scan_string(scan_info *info,int max_len)
{
...
int expect(scan_info *info,int check)
{
...
int scan_info_next_line(scan_info *info)
{
    if(info->current == NULL) return 0;
...
int scan_info_home(scan_info *info)
{
...
int set_scan_info_line(scan_info *info)
{

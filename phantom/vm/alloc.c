...
void * pvm_object_space_start;
void * pvm_object_space_end;
...
static inline unsigned int round_size(unsigned int size, persistent_arena_t *ap)
{
    (void) ap;
...
static void alloc_collapse_with_next_free(pvm_object_storage_t *op, unsigned int need_size, void * end, persistent_arena_t *ap)
{
    assert( op->_ah.object_start_marker == PVM_OBJECT_START_MARKER );
    assert( op->_ah.alloc_flags == PVM_OBJECT_AH_ALLOCATOR_FLAG_FREE );
...
void pvm_collapse_free(pvm_object_storage_t *op)
{
    ...
    persistent_arena_t *ap = find_arena_by_address(op);
    if( ap == 0 )
        return;
...
static pvm_object_t pvm_find(unsigned int size, persistent_arena_t *ap)
{

#define CURR_POS  (ap->curr)
...
//    }
    ...
    pvm_object_t start = ap->base;
    pvm_object_t end = ap->base + ap->size;
    ...
    while(result == 0)
    {
        ...
        if( PVM_OBJECT_AH_ALLOCATOR_FLAG_ALLOCATED & curr->_ah.alloc_flags )
        {
            ...
            curr = alloc_wrap_to_next_object(curr, start, end, &wrap, ap);
            continue;
        }
        ...
        if (curr->_ah.exact_size < size) {
            alloc_collapse_with_next_free(curr, size, end, ap);
            // try again -
            if (curr->_ah.exact_size < size) {
                DEBUG_PRINT("|");
                curr = alloc_wrap_to_next_object(curr, start, end, &wrap, ap);
    ...
    CURR_POS = alloc_wrap_to_next_object(result, start, end, &wrap, ap);
    //CURR_POS = result;
    return result;
}
...
static pvm_object_storage_t * pool_alloc(unsigned int size, persistent_arena_t *ap)
{
    ...
    do {
        ...
        printf("\n out of mem looking for %d bytes, arena %p. Run GC... \n", size, ap);
...
pvm_object_storage_t * pvm_object_alloc( unsigned int data_area_size, unsigned int flags, bool saturated )
{
    ...
    if( data == 0 )
    {
        ...
        pvm_memcheck();
        // TODO VM throw
        panic("out of persistent mem looking for %d bytes, arena %p", size, ap );
    }
...
/*
static void memcheck_print_histogram(unsigned persistent_arena_t *ap)
{

    ...
    ACPI_SIZE               *TableAddress)
{
    ...
    ACPI_MOVE_16_TO_32 (&PhysicalAddress, TablePtr+ACPI_EBDA_PTR_OFFSET);
    ...
    {
		int offset = PhysicalAddress - PREV_PAGE_ALIGN(PhysicalAddress);
        /*
         * 1b) Search EBDA paragraphs (EBDA is required to be a
         *     minimum of 1K length)
         */
        TablePtr = AcpiOsMapMemory (
                    (ACPI_PHYSICAL_ADDRESS) PhysicalAddress-offset,
                    ACPI_EBDA_WINDOW_SIZE+offset);
        if (!TablePtr)
        {
            ACPI_ERROR ((AE_INFO,
                "Could not map memory at 0x%8.8X for length %u",
        ...
        MemRover = AcpiTbScanMemoryForRsdp (TablePtr+offset, ACPI_EBDA_WINDOW_SIZE);

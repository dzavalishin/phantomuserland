...
static int set_up_xfer( ataio_t *ata, int dir, long bc, physaddr_t phyAddr )

{
    ...
    sub_writeBusMstrCmd( ata, BM_CR_MASK_STOP );
    sub_writeBusMstrStatus( ata, ata->statReg | BM_SR_MASK_INT | BM_SR_MASK_ERR );
...

    {
        ...
        prdPtr = ata->dma_pci_prd_va; //prdBufPtr;
        /*
    ...
    ata->dma_pci_num_prd = numPrd;
    outw( ata->pio_bmide_base_addr + BM_PRD_ADDR_LOW,
          (unsigned int) ( ata->dma_pci_prd_pa & 0x0000ffffL ) );
    outw( ata->pio_bmide_base_addr + BM_PRD_ADDR_HIGH,
          (unsigned int) (( ata->dma_pci_prd_pa >> 16 ) & 0x0000ffffL ) );
...
#warning long here meant 32 bit?
    ...
    if ( dir )
        ata->rwControl = BM_CR_MASK_READ;     // ATA Write DMA
    else
        ata->rwControl = BM_CR_MASK_WRITE;    // ATA Read DMA
    sub_writeBusMstrCmd( ata, ata->rwControl );
    return 0;
}
...
int dma_pci_config( ataio_t *ata, unsigned int regAddr )
{
    ...
    // ERROR must be on 64K boundary!?
    hal_pv_alloc( &(ata->dma_pci_prd_pa), &(ata->dma_pci_prd_va), PRD_BUF_SIZE );
    ...
    ata->dma_pci_num_prd = 0;

    // read the BM status reg and save the upper 3 bits.
    ata->statReg = sub_readBusMstrStatus(ata) & 0x60;
...
static int exec_pci_ata_cmd( ataio_t *ata, struct REG_CMD_INFO *reg_cmd_info_p, int dev, physaddr_t physAddr,
...
//    struct REG_CMD_INFO reg_cmd_info = *reg_cmd_info_p; // Ok?
    ...
    sub_readBusMstrCmd(ata);
    sub_readBusMstrStatus(ata);
    sub_writeBusMstrCmd( ata, ata->rwControl | BM_CR_MASK_START );
    sub_readBusMstrCmd(ata);
    sub_readBusMstrStatus(ata);
    ...
    while ( 1 )
    {
        ...
        if ( ! ( cntr & 0x1fff ) )
        {
            sub_readBusMstrStatus(ata);         // read BM status (for trace)
        ...
        if ( ata->int_intr_flag )                // interrupt ?
        {
            ...
            trc_llt( CB_STAT, ata->int_ata_status, TRC_LLT_INB );
    ...
    sub_writeBusMstrCmd( ata, BM_CR_MASK_STOP );    // shutdown DMA
    sub_readBusMstrCmd(ata);                      // read BM cmd (just for trace)
    status |= sub_readBusMstrStatus(ata);         // read BM status again
...
int dma_pci_lba28( ataio_t *ata, int dev, int cmd,
...
//    struct REG_CMD_INFO reg_cmd_info;
    ...
    return exec_pci_ata_cmd( ata, &reg_cmd_info, dev, (int)addr, numSect );
}
...
int dma_pci_lba48( ataio_t *ata, int dev, int cmd,
                   unsigned int fr, unsigned int sc,
...
//    struct REG_CMD_INFO reg_cmd_info;
    ...
    return exec_pci_ata_cmd( ata, &reg_cmd_info, dev, physAddr, numSect );
}
...
int dma_pci_packet( ataio_t *ata, int dev,
                    unsigned int cpbc,
...
//    struct REG_CMD_INFO reg_cmd_info;
    ...
    reg_cmd_info.fr1 = ata->reg_atapi_reg_fr | 0x01;  // packet DMA mode !
    ...
    if ( set_up_xfer( ata, dir, dpbc, dp_physAddr ) )
    {
        reg_cmd_info.ec = 61;
        trc_llt( 0, reg_cmd_info.ec, TRC_LLT_ERROR );
        sub_trace_command(ata);
    ...
    if ( reg_cmd_info.ec == 0 )
    {
        ...
        sub_writeBusMstrCmd( ata, BM_CR_MASK_STOP );    // shutdown DMA
        sub_readBusMstrCmd(ata);                      // read BM cmd (just for trace)
        status |= sub_readBusMstrStatus(ata);         // read BM statu again

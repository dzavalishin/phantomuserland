...
// TODO too long!
#define idedelay() phantom_spinwait(1)
...
//	int nsects;                          /* Number of sectors left to transfer */
	...
	physaddr_t prds_phys;             /* Physical address of PRD list */

        //struct waitq waitq;
        hal_cond_t waitq;

        int             have_dev[2]; // move to hd_t
        hd_t            hdtab[2];

} hdc_t;


//typedef struct ide_tab {	hd_t *drive[HD_DRIVES];} ide_tab_t;

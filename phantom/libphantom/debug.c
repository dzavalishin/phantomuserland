            ...
            case 66: // down arrow
                {
                    ...
                    if(ptr > 0)
                        printf("\x1b[%dD", ptr); // move to beginning of line

                    strcpy(buf, line_buf[history_line]);
                    ptr = strlen(buf);
                    printf("%s\x1b[K", buf); // print the line and clear the rest
...
static void kernel_debugger_loop()
{
    ...
    printf("kernel debugger on cpu %d\n", GET_CPU_ID());
...
int dbg_add_command(void (*func)(int, char **), const char *name, const char *desc)
{
    ...
    //acquire_spinlock(&dbg_spinlock);
    int ie = hal_save_cli();
    hal_spin_lock(&dbg_spinlock);

    cmd->next = commands;
    commands = cmd;

    hal_spin_unlock(&dbg_spinlock);
    //release_spinlock(&dbg_spinlock);
    //int_restore_interrupts();
    if(ie) hal_sti();
    }
